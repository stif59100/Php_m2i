...
class PaysDAO {
     ...
     * @return int
     */
    public static function insert(PDO $pdo, Pays $pays): int {
        ...
        try {
            ...
            $lcmd = $pdo->prepare("INSERT INTO pays(id_pays, nom_pays) VALUES(?,?)");
            // On associe des valeurs aux paramètres de la requete, ie les attributs de l'objet reçu en param
...
//            echo $exc->getMessage();
     ...
     * @return int
     */
    public static function delete(PDO $pdo, Pays $pays): int {
        $affected = 0;

        try {
            $lcmd = $pdo->prepare("DELETE FROM pays WHERE id_pays = ?");
            $lcmd->bindValue(1, $pays->getIdPays());
            $lcmd->execute();
            $affected = $lcmd->rowCount();
        } catch (Exception $exc) {
//            echo $exc->getMessage();
     ...
     * @return int
     */
    public static function update(PDO $pdo, Pays $pays): int {
        $affected = 0;

        try {
            $lcmd = $pdo->prepare("UPDATE pays SET nom_pays = ? WHERE id_pays = ?");
...
//            echo $exc->getMessage();
     ...
     * @return \Pays
     */
    public static function selectOne(PDO $pdo, $pk) {

        $message = "Bonjour la locale";
        
        try {
            $lrs = $pdo->prepare("SELECT * FROM pays WHERE id_pays = ?");
            $lrs->bindParam(1, $pk);
            $lrs->execute();
            $record = $lrs->fetch();
            if ($record != null) {
...
//        return $tPays;
        try {
            ...
            $lrs = $pdo->query("SELECT id_pays AS idPays, nom_pays AS nomPays FROM pays");
